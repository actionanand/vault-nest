...
The build log and GitHub job summary show emoji-labelled artifact results such as `SIGNED APK`, `UNSIGNED APK`, `SIGNED AAB`, `UNSIGNED AAB`, `R8 mapping`, and `Native debug symbols`, including the generated paths.
...
| `.github/workflows/android-build.yml` | Builds, optionally signs, verifies, summarizes, and uploads APK/AAB, R8 mapping, and native symbol files               |
...
| `scripts/patch-android.mjs`           | Adds native features, enables optimization/symbol generation, and protects WebView JavaScript interfaces               |
