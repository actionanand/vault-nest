...
Credential copy shortcuts use `@capacitor/local-notifications` for permission checks and native Android clipboard handling for notification taps. Permission is requested only when the user invokes the Android-only notification action. Selected username, email, and password values remain available from those notifications for three minutes after sending, including while the vault is locked; notification text and pending intents never contain the values. Android Keystore AES-GCM protects the temporary values in app-private storage. A non-exported, no-display activity handles every notification tap in a foreground-authorized context, updates the clipboard with the selected field, and immediately returns focus to the other app. The native shell schedules notification dismissal at the three-minute expiry plus clipboard clearing at the five-minute clipboard window.
...
5. Generates padded launcher, round, foreground, notification, store, and branded splash images from `public/vault-nest.png`.
...
`scripts/patch-android.mjs` adds the backup document picker, Android Keystore biometric bridge, screenshot-protection bridge, native splash overlay, light/dark system-bar bridge, encrypted credential-shortcut store, no-display credential-copy activity, biometric permission and dependency, and a monochrome status-bar notification vector. Android notification icons must be white-alpha silhouettes; the notification accent color follows Vault Nest's resolved light/dark theme. The patch pins `MainActivity` to a `Theme.SplashScreen` launch theme and writes the Android 12+ splash attributes so cold starts render the Vault Nest brand image instead of a plain system window.
